"""Test single-cell vs bulk data: branching pathways with stimulus."""
import numpy as np
...
np.savetxt(result_path + 'data1.txt', data1, fmt='%d', delimiter='\t')
np.savetxt(result_path + 'data2.txt', data2, fmt='%d', delimiter='\t')
...
for i, data in [(1, data1), (2, data2)]:
    ...
    fig.savefig(result_path + f'mean{i}.pdf', bbox_inches='tight')
...
for i, model in [(1, model1), (2, model2)]:
    ...
    fig.savefig(result_path + f'graph{i}.pdf', bbox_inches='tight')
...
for i in [1, 2]:
    # Load the data
    x = np.loadtxt(result_path + f'data{i}.txt', dtype=int, delimiter='\t')
    # Calibrate the model
    model = NetworkModel()
    model.fit(x)
    # Export interaction matrix
    np.savetxt(result_path + f'inter{i}.txt', model.inter, delimiter='\t')
